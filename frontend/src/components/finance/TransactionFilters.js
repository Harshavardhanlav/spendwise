...
function TransactionFilters({ filters, categories, search, onChange, onSearch, onReset }) {
  return <div className="transaction-filters">
    ...
    <label className="date-filter"><span>Start date</span><input type="date" value={filters.startDate} onChange={(event) => onChange('startDate', event.target.value)} aria-label="Start date" /></label>
    <label className="date-filter"><span>End date</span><input type="date" value={filters.endDate} onChange={(event) => onChange('endDate', event.target.value)} aria-label="End date" /></label>
